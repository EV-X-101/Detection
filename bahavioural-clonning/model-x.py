...
# Load the behavioural cloning model
...
# Function for behavioural cloning model
def predict(image):
...
while True:
    ...
    # Perform inference on the captured frame with YOLO model
    results = yolo_model(frame)
    
    # Perform steering angle prediction with behavioural cloning model
    turn_command = predict(frame)
    ...
    for item in detected_info:
        ...
        if class_name == 'traffic light':
            ...
            if traffic_light_color == 'red' or traffic_light_color == 'yellow':
                ...
                sock.sendall(b'stop')
    # Send command to Raspberry Pi to move car forward or stop if no obstacle is detected
    if not obstacle_detected:
        print("No obstacle detected. Moving car forward.")
        sock.sendall('forward'.encode())
        sock.sendall(turn_command.encode())
