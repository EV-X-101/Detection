...
# Load your trained behavioral cloning model for road segmentation and vehicle correction
model_path = 'model51.h5'
behavioral_model = torch.load(model_path)
...
def process_frame(image):
    ...
    # Perform road segmentation and vehicle correction using your model
    with torch.no_grad():
        segmented_image = behavioral_model(image)

    # Convert the segmented image back to NumPy array and normalize
    segmented_image = segmented_image.squeeze(0).numpy().transpose((1, 2, 0))
    segmented_image = (segmented_image * 255.0).astype(np.uint8)
...
while True:
    ...
    # Perform road segmentation and vehicle correction
    ...
    for item in detected_info:
        ...
        cv2.rectangle(processed_frame, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (255, 0, 0), 2)
        cv2.putText(processed_frame, text, (int(x_min), int(y_min) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
        ...
        if class_name == 'traffic light':
            ...
            traffic_light_roi = processed_frame[int(y_min):int(y_max), int(x_min):int(x_max)]

            # Get the state of the traffic light
            traffic_light_color = traffic_light_state(processed_frame, (x_min, y_min, x_max, y_max))

            # Render the traffic light color on the frame
            cv2.rectangle(processed_frame, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (0, 0, 255), 2)
            cv2.putText(processed_frame, f"Color: {traffic_light_color}", (int(x_min), int(y_min) + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
